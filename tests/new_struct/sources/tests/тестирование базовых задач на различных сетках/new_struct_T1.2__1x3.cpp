...
//    };
    ...
    if (1)
    {
		...
		static char *MeshFileName = "1x3_T1.2.msh";
//		static char *MeshFileName = "1x3_T1.2_nestr.msh";
...
//        U[0][y] = [mu, nu, c0] (const dealii::Point<2> &p) {return mu*nu*(p(0)-c0)*p(1);}; //Uy
        ...
        U[0][y] = [mu, nu, c0] (const dealii::Point<2> &p) {return mu*nu*(p(0)-c0)*p(1);}; //Uy
        U[1][x] = [mu, nu, c0, C0] (const dealii::Point<2> &p) {return mu*nu*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0)+2.0*C0);};
