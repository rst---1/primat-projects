...
dbl Analytic_function (const dealii::Point<2> p, cst n)
{
...
int main()
{
    ...
    if (1)
    {
		...
		static char *MeshFileName = "1x1_T1.1.msh";
//		static char *MeshFileName = "1x1_T1.1_nestr.msh";
...
//		std::ifstream f("1x3_T1.2_nestr.msh");
...
//            element_matrix.C[1][y][y] = 0.4;
        ...
        cdbl mu = 1.0;
        vec<arr<typename Nikola::SourceScalar<2>::Func, 2>> U(2);
        U[0][x] = [mu, nu, c0] (const dealii::Point<2> &p) {return 1.0;}; //Ux
...
//								если он не существует.
        ...
        for (auto cell = domain.dof_handler.begin_active (); cell != domain.dof_handler.end (); ++cell)
        {
            for (st i = 0; i < dealii::GeometryInfo<2>::vertices_per_cell; ++i)
            {
                ...
                analytic(indx) = Analytic_function(cell->vertex(i), 32);
