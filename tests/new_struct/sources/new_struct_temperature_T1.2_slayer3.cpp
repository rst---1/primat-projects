...
namespace NEW_STRUCT_TEMPERATURE_T1_2
{
...
/*
	        U[0][x] = [mu1, Puasson1, c0, bb] (const dealii::Point<2> &p) {return mu1*Puasson1*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + (bb*bb*bb-1.0)/12);}; //Ux
	        U[0][y] = [mu1, Puasson1, c0] (const dealii::Point<2> &p) {return mu1*Puasson1*(p(0)-c0)*p(1);}; //Uy
	        U[1][x] = [mu2, Puasson2, c0, bb] (const dealii::Point<2> &p) {return mu2*Puasson2*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + (bb*bb*bb-1.0)/12);}; //Ux
	        U[1][y] = [mu2, Puasson2, c0] (const dealii::Point<2> &p) {return mu2*Puasson2*(p(0)-c0)*p(1);}; //Uy
	        U[2][x] = [mu3, Puasson3, c0, bb] (const dealii::Point<2> &p) {return mu3*Puasson3*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + (bb*bb*bb-1.0)/12);}; //Ux
	        U[2][y] = [mu3, Puasson3, c0] (const dealii::Point<2> &p) {return mu3*Puasson3*(p(0)-c0)*p(1);}; //Uy
*/
	        U[0][x] = [mu1, Puasson1, c0, C0] (const dealii::Point<2> &p) {return mu1*Puasson1*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + 2*C0 );}; //Ux
	        U[0][y] = [mu1, Puasson1, c0] (const dealii::Point<2> &p) {return mu1*Puasson1*(p(0)-c0)*p(1);}; //Uy
	        U[1][x] = [mu2, Puasson2, c0, C0] (const dealii::Point<2> &p) {return mu2*Puasson2*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + 2*C0 );}; //Ux
	        U[1][y] = [mu2, Puasson2, c0] (const dealii::Point<2> &p) {return mu2*Puasson2*(p(0)-c0)*p(1);}; //Uy
	        U[2][x] = [mu3, Puasson3, c0, C0] (const dealii::Point<2> &p) {return mu3*Puasson3*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0) + 2*C0 );}; //Ux
	        U[2][y] = [mu3, Puasson3, c0] (const dealii::Point<2> &p) {return mu3*Puasson3*(p(0)-c0)*p(1);}; //Uy


		    vec<typename Nikola::SourceScalar<2>::Func> tau(3);
        	tau[0] = [Yung1, c0] (const dealii::Point<2> &p) {return Yung1*(p(0)-c0);};
        	tau[1] = [Yung2, c0] (const dealii::Point<2> &p) {return Yung2*(p(0)-c0);};
        	tau[2] = [Yung3, c0] (const dealii::Point<2> &p) {return Yung3*(p(0)-c0);};
	...
	//-------------------------------------
					...
					if(point(0) < h)	{
											AU_z(i) = Analytic_functionU_z(point, 15, Puasson1, c0, C0, bb);
										}
					else if(point(0) <= 2*h)	{
											AU_z(i) = Analytic_functionU_z(point, 15, Puasson2, c0, C0, bb);
										}
					else				{
											AU_z(i) = Analytic_functionU_z(point, 15, Puasson3, c0, C0, bb);
										}
	...
	//-------------------------------------
					...
					if(point(0) < h)	{
											AU_z_gradX(i) = Analytic_functionU_z_gradX(point, 15, Puasson1, c0, C0, bb);
											AU_z_gradY(i) = Analytic_functionU_z_gradY(point, 15, Puasson1, c0, C0, bb);
										}
					else if(point(0) <= 2*h)	{
											AU_z_gradX(i) = Analytic_functionU_z_gradX(point, 15, Puasson2, c0, C0, bb);
											AU_z_gradY(i) = Analytic_functionU_z_gradY(point, 15, Puasson2, c0, C0, bb);
										}
					else				{
											AU_z_gradX(i) = Analytic_functionU_z_gradX(point, 15, Puasson3, c0, C0, bb);
											AU_z_gradY(i) = Analytic_functionU_z_gradY(point, 15, Puasson3, c0, C0, bb);
										}
	...
	//-------------------------------------
					...
					if(point(0) < h)	{
											Atau_zx(i) = Analytic_functionTau_zx(point, 15, Puasson1, c0, C0, bb, mu1);
											Atau_zy(i) = Analytic_functionTau_zy(point, 15, Puasson1, c0, C0, bb, mu1);
										}
					else if(point(0) <= 2*h)	{
											Atau_zx(i) = Analytic_functionTau_zx(point, 15, Puasson2, c0, C0, bb, mu2);
											Atau_zy(i) = Analytic_functionTau_zy(point, 15, Puasson2, c0, C0, bb, mu2);
										}
					else				{
											Atau_zx(i) = Analytic_functionTau_zx(point, 15, Puasson3, c0, C0, bb, mu3);
											Atau_zy(i) = Analytic_functionTau_zy(point, 15, Puasson3, c0, C0, bb, mu3);
										}
