...
reader_6 = CSVReader(FileName = [DIRECTORY_OF_IN_XY_A])
...
for i_Files in range(len(Slice_x_)):
	...
	Render()
	writer = CreateWriter(DIRECTORY_OF_OUT + "/" + CSV_FILENAME_2 + " tau_xy when x("+str(Slice_x_[i_Files])+").csv")
...
for i_Files in range(len(Slice_y_)):
	...
	Render()
	writer = CreateWriter(DIRECTORY_OF_OUT + "/" + CSV_FILENAME_2 + " tau_xy when y("+str(Slice_y_[i_Files])+").csv")
	writer.WriteAllTimeSteps = 1
	writer.FieldAssociation = "Points"
	writer.UpdatePipeline()
#---------------------------------------------------------
