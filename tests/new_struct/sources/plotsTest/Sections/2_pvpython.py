...
for NUMBER_OF_MESH in range(len(NAME_OF_MESH)):
	for NUMBER_OF_FUNCTION in range(len(NAME_OF_FUNCTION)):
		#open file
		from paraview.simple import *
		reader_1 = CSVReader(FileName = [DIRECTORY_OF_FUNCTION[NUMBER_OF_FUNCTION]])
...
#			writer = CreateWriter(DIRECTORY_OF_OUT + "/" + CSV_FILENAME_1 + " " + NAME_OF_FUNCTION[NUMBER_OF_FUNCTION] + " when x("+str(Slice_x_[i_Files])+").csv")
			writer = CreateWriter(DIRECTORY_OF_MOUTTTTT + "/" + NAME_OF_MESH[NUMBER_OF_MESH] + "/" + CSV_FILENAME_1 + " " + NAME_OF_FUNCTION[NUMBER_OF_FUNCTION] + " when x("+str(Slice_x_[i_Files])+").csv")
...
#			writer = CreateWriter(DIRECTORY_OF_OUT + "/" + CSV_FILENAME_1 + " " + NAME_OF_FUNCTION[NUMBER_OF_FUNCTION] + " when y("+str(Slice_y_[i_Files])+").csv")
			writer = CreateWriter(DIRECTORY_OF_MOUTTTTT + "/" + NAME_OF_MESH[NUMBER_OF_MESH] + "/" + CSV_FILENAME_1 + " " + NAME_OF_FUNCTION[NUMBER_OF_FUNCTION] + " when y("+str(Slice_y_[i_Files])+").csv")
			writer.WriteAllTimeSteps = 1
			writer.FieldAssociation = "Points"
			writer.UpdatePipeline()
		#---------------------------------------------------------
