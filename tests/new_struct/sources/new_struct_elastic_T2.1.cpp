...
//		    U[0][y] = [&U_z, &domain, &element_matrix, c0] (const dealii::Point<2> &p) {return element_matrix.C[0][x][x][z][z] * HCPTools::print_function_elastic<2>(U_z, domain, p);}; //Uz					//INPUT
		    ...
		    U[0][y] = [&element_matrix, c0] (const dealii::Point<2> &p) {return element_matrix.C[0][x][x][z][z] * (-1)*(p(0)-c0);}; //Uz					//INPUT
...
//								+	Yung * (-1) * (point(0) - c0);
	...
	//-------------------------------------			//INPUT
					...
					AU_x(i) = Puasson / 2 * ( -point(1)*point(1) + (point(0) - c0)*(point(0) - c0) + 2*C0 );
