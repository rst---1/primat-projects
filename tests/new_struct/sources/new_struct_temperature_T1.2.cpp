...
//			READ_FILE_TO_ARRAY::read_vector("out/Ftau_zz_81.gpl", tau_zz, 3, 4);		//Считываем в новый вектор данные из файла. ...3, 6 - четвёртый столбец из шести
		    ...
		    U[0][x] = [mu,  &element_matrix, Puasson, bb, c0] (const dealii::Point<2> &p) {return element_matrix.C[0][x][x]*0.5*Puasson*( -p(1)*p(1) + (p(0)-c0)*(p(0)-c0) + (bb*bb*bb-1.0)/12 );};		//Ux
		    U[0][y] = [mu,  &element_matrix, Puasson, bb, c0] (const dealii::Point<2> &p) {return element_matrix.C[0][y][y]*Puasson*p(1)*(p(0)-c0) ;};		//Uy
...
//	        U[0][x] = [mu, Puasson, c0] (const dealii::Point<2> &p) {return mu*Puasson*0.5*(std::pow(p(0)-c0,2.0)-std::pow(p(1),2.0));}; //Ux
//	        U[0][y] = [mu, Puasson, c0] (const dealii::Point<2> &p) {return mu*Puasson*(p(0)-c0)*p(1);}; //Uy
...
//	        tau[0] = [Yung,  &element_matrix, c0] (const dealii::Point<2> &p) {return -element_matrix.C[0][x][x]*Yung*(p(0)-c0); };														//tau_zz
        	tau[0] = [Yung, c0] (const dealii::Point<2> &p) {return Yung*(p(0)-c0);};
...
//				printf("\t\tnumber_of_dots = %d\n", number_of_dots);
	...
	//-------------------------------------
					...
					AU_z(i) = Analytic_functionU_z(point, 15, Puasson, c0, C0, bb);
					fprintf( FAU_z, "%8d\t\t%5.35f\t\t%5.35f\t\t%5.35f\r\n", i, point(0), point(1), AU_z(i)  );

	//-------------------------------------

					AU_z_gradX(i) = Analytic_functionU_z_gradX(point, 15, Puasson, c0, C0, bb);
					AU_z_gradY(i) = Analytic_functionU_z_gradY(point, 15, Puasson, c0, C0, bb);
	...
	//-------------------------------------
					...
					Atau_zx(i) = Analytic_functionTau_zx(point, 15, Puasson, c0, C0, bb, mu);
					Atau_zy(i) = Analytic_functionTau_zy(point, 15, Puasson, c0, C0, bb, mu);
