...
namespace NEW_STRUCT_TEMPERATURE
{
	...
	double Integral(dealii::Vector<dbl> &Vec, Domain<2> &domain, dealii::FEValues<2> &fe_values, cst &n_q_points, cst &dofs_per_cell)						//Позволяет добавить к решению константу
	{
...
//		    dealii::SolverControl solver_control (10000, 1e-2);
	...
	//Вычисление интеграла
				{
					...
					double rx = sqrt(AveArea) / 5;
					double ry = sqrt(AveArea) / 5;
