...
namespace NEW_STRUCT_ELASTIC_T2_2 {
            ...
            double Ycentr, double rx, double ry, dealii::Point<2, double> point) //Поиск вершину, близкой к центру (Xcentr,Ycentr)
    {
        ...
        for (int i = 0; i < U_z.size(); ++i) //Пробежим по всем точкам, измеряя расстояние между данной точкой и центром
        {
            ...
            if ((point(0) - Xcentr)*(point(0) - Xcentr) + (point(1) - Ycentr)*(point(1) - Ycentr) <= rx * ry) {
                ID = i;
            }
        }
        return ID;
    ...
    void Errors(const int &i, dealii::Vector<dbl> &Vec1, dealii::Vector<dbl> &Vec2,
        ...
        if ((point(0) > Xdown)&&(point(0) < Xup) && (point(1) > Yleft)&&(point(1) < Yright)
            ...
            if (Vec2(i) != 0.0) {
                deltOTN = abs(deltABS / Vec2(i) * 100.0);
            } else deltOTN = 0.0;
            if (deltMaxABS < deltABS) {
                deltMaxABS = deltABS;
            }
            if (deltMaxOTN < deltOTN) {
                deltMaxOTN = deltOTN;
            }
        }
        /*
    ...
    int main(const double &Yung1,
        ...
        if (1) {
            ...
            EPTools::set_isotropic_elascity{yung : Yung1, puasson : Puasson1}
            (element_matrix.C[0]);
            EPTools::set_isotropic_elascity{yung : Yung2, puasson : Puasson2}
            (element_matrix.C[1]);
            EPTools::set_isotropic_elascity{yung : Yung3, puasson : Puasson3}
            (element_matrix.C[2]);
             ...
             */
            U[0][x] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[0][x][x][z][z] * 1.0;
            }; //Uz					//INPUT
            U[0][y] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[0][x][x][z][z] * 1.0;
            }; //Uz					//INPUT				//INPUT
            U[1][x] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[1][x][x][z][z] * 1.0;
            }; //Uz					//INPUT
            U[1][y] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[1][x][x][z][z] * 1.0;
            }; //Uz					//INPUT
            U[2][x] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[2][x][x][z][z] * 1.0;
            }; //Uz					//INPUT
            U[2][y] = [&element_matrix, c0] (const dealii::Point<2> &p) {
                return element_matrix.C[2][x][x][z][z] * 1.0;
            }; //Uz					//INPUT
             ...
             */
            tau[0][x] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zx													//INPUT
            tau[0][y] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zy													//INPUT
            tau[1][x] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zx													//INPUT
            tau[1][y] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zy													//INPUT
            tau[2][x] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zx													//INPUT
            tau[2][y] = [] (const dealii::Point<2> &p) {
                return 0.0;
            }; //tau_zy													//INPUT
...

            {
                ...
                for (int i = 0; i < slae.solution.size() / 2; ++i) {
                    ...
                    if ((point(0) > Xdown)&&(point(0) < Xup) && (point(1) > Yleft)&&(point(1) < Yright)) {
                        deltABSU_x = abs(AU_x(i) - U_x(i));
                        if (U_x(i) != 0.0) {
                            deltOTNU_x = abs(deltABSU_x / U_x(i) * 100.0);
                        } else deltOTNU_x = 0.0;
                        if (deltMaxABSU_x < deltABSU_x) {
                            deltMaxABSU_x = deltABSU_x;
                        }
                        if (deltMaxOTNU_x < deltOTNU_x) {
                            deltMaxOTNU_x = deltOTNU_x;
                        }

                        deltABSU_x_gradX = abs(AU_x_gradX(i) - U_x_gradX(i));
                        if (U_x_gradX(i) != 0.0) {
                            deltOTNU_x_gradX = abs(deltABSU_x_gradX / U_x_gradX(i) * 100.0);
                        } else deltOTNU_x_gradX = 0.0;
                        if (deltMaxABSU_x_gradX < deltABSU_x_gradX) {
                            deltMaxABSU_x_gradX = deltABSU_x_gradX;
                        }
                        if (deltMaxOTNU_x_gradX < deltOTNU_x_gradX) {
                            deltMaxOTNU_x_gradX = deltOTNU_x_gradX;
                        }

                        deltABSU_x_gradY = abs(AU_x_gradY(i) - U_x_gradY(i));
                        if (U_x_gradY(i) != 0.0) {
                            deltOTNU_x_gradY = abs(deltABSU_x_gradY / U_x_gradY(i) * 100.0);
                        } else deltOTNU_x_gradY = 0.0;
                        if (deltMaxABSU_x_gradY < deltABSU_x_gradY) {
                            deltMaxABSU_x_gradY = deltABSU_x_gradY;
                        }
                        if (deltMaxOTNU_x_gradY < deltOTNU_x_gradY) {
                            deltMaxOTNU_x_gradY = deltOTNU_x_gradY;
                        }
                    }
                    ...
                    if ((point(0) > Xdown)&&(point(0) < Xup) && (point(1) > Yleft)&&(point(1) < Yright)) {
                        ...
                        if (U_y(i) != 0.0) {
                            deltOTNU_y = abs(deltABSU_y / U_y(i) * 100.0);
                        } else deltOTNU_y = 0.0;
                        if (deltMaxABSU_y < deltABSU_y) {
                            deltMaxABSU_y = deltABSU_y;
                        }
                        if (deltMaxOTNU_y < deltOTNU_y) {
                            deltMaxOTNU_y = deltOTNU_y;
                        }

                        deltABSU_y_gradX = abs(AU_y_gradX(i) - U_y_gradX(i));
                        if (U_y_gradX(i) != 0.0) {
                            deltOTNU_y_gradX = abs(deltABSU_y_gradX / U_y_gradX(i) * 100.0);
                        } else deltOTNU_y_gradX = 0.0;
                        if (deltMaxABSU_y_gradX < deltABSU_y_gradX) {
                            deltMaxABSU_y_gradX = deltABSU_y_gradX;
                        }
                        if (deltMaxOTNU_y_gradX < deltOTNU_y_gradX) {
                            deltMaxOTNU_y_gradX = deltOTNU_y_gradX;
                        }

                        deltABSU_y_gradY = abs(AU_y_gradY(i) - U_y_gradY(i));
                        if (U_y_gradY(i) != 0.0) {
                            deltOTNU_y_gradY = abs(deltABSU_y_gradY / U_y_gradY(i) * 100.0);
                        } else deltOTNU_y_gradY = 0.0;
                        if (deltMaxABSU_y_gradY < deltABSU_y_gradY) {
                            deltMaxABSU_y_gradY = deltABSU_y_gradY;
                        }
                        if (deltMaxOTNU_y_gradY < deltOTNU_y_gradY) {
                            deltMaxOTNU_y_gradY = deltOTNU_y_gradY;
                        }
                    }
                    ...
                    if ((point(0) > Xdown)&&(point(0) < Xup) && (point(1) > Yleft)&&(point(1) < Yright)) {
                        ...
                        if (tau_xx(i) != 0.0) {
                            deltOTNtau_xx = abs(deltABStau_xx / tau_xx(i) * 100.0);
                        } else deltOTNtau_xx = 0.0;
                        if (deltMaxABStau_xx < deltABStau_xx) {
                            deltMaxABStau_xx = deltABStau_xx;
                        }
                        if (deltMaxOTNtau_xx < deltOTNtau_xx) {
                            deltMaxOTNtau_xx = deltOTNtau_xx;
                        }

                        deltABStau_yy = abs(Atau_yy(i) - tau_yy(i));
                        deltAVEtau_yy += deltABStau_yy;
                        if (tau_yy(i) != 0.0) {
                            deltOTNtau_yy = abs(deltABStau_yy / tau_yy(i) * 100.0);
                        } else deltOTNtau_yy = 0.0;
                        if (deltMaxABStau_yy < deltABStau_yy) {
                            deltMaxABStau_yy = deltABStau_yy;
                        }
                        if (deltMaxOTNtau_yy < deltOTNtau_yy) {
                            deltMaxOTNtau_yy = deltOTNtau_yy;
                        }

                        deltABStau_xy = abs(Atau_xy(i) - tau_xy(i));
                        deltAVEtau_xy += deltABStau_xy;
                        if (tau_xy(i) != 0.0) {
                            deltOTNtau_xy = abs(deltABStau_xy / tau_xy(i) * 100.0);
                        } else deltOTNtau_xy = 0.0;
                        if (deltMaxABStau_xy < deltABStau_xy) {
                            deltMaxABStau_xy = deltABStau_xy;
                        }
                        if (deltMaxOTNtau_xy < deltOTNtau_xy) {
                            deltMaxOTNtau_xy = deltOTNtau_xy;
                        }
                    }
                    ...
                    if ((point(0) > Xdown)&&(point(0) < Xup) && (point(1) > Yleft)&&(point(1) < Yright)) {
                        ...
                        if (tau_zz(i) != 0.0) {
                            deltOTNtau_zz = abs(deltABStau_zz / tau_zz(i) * 100.0);
                        } else deltOTNtau_zz = 0.0;
                        if (deltMaxABStau_zz < deltABStau_zz) {
                            deltMaxABStau_zz = deltABStau_zz;
                        }
                        if (deltMaxOTNtau_zz < deltOTNtau_zz) {
                            deltMaxOTNtau_zz = deltOTNtau_zz;
                        }
                    }
