...
set size 0.99, 0.99
set xrange [ 0 : 1 ]
set yrange [ 0.10 : 0.28 ]
set xtics 0,0.1,1#0,1024,16384
set ytics 0.10, 0.02, 0.28
set key spacing 3.2
set key top center
# \n Рис.3. Сравнение макротеплопроводности ячейки для \n включений различной формы
set xlabel "Коэффициент армирования {/Symbol \161}" font "Monospace, 30"
...
plot \
 "../concrete/mata-quadrate.gpd" using ($1/16384):($3) w l ls 1 ti "{/Arial=25 1}",\
 "../concrete/mata-shell.gpd" using ($1/16384):($3) w l ls 2 ti "{/Arial=25 2}",\
 "../concrete/mata-cross.gpd" using ($1/16384):($3) w l ls 3 ti "{/Arial=25 3}", \
 "../concrete/mata-circ.gpd" using ($1/16384):($3) w lp ls 4 ti "{/Arial=25 4}"
