...
set ylabel "Коэффициент Пуассона {/Symbol n_{/Nosymbol zx}}" font "Serif, 25"# offset 2
...
plot \
 dir."mata-quadrate.gpd" using ($1/16384):($4) w l ls 1 ti "{/Serif=15 квадрат}",\
 dir."mata-shell.gpd" using ($1/16384):($4) w l ls 2 ti "{/Serif=15 трубка}",\
 dir."mata-cross.gpd" using ($1/16384):($4) w l ls 3 ti "{/Serif=15 крестовина}", \
 dir."mata-circ.gpd" using ($1/16384):($4) w l ls 4 ti "{/Serif=15 круг}", \
 "../../ch_fork.gpd" using ($1/16384):($4) w lp ls 5 ti "{/Serif=15 нижня оценка}" , \
 "../../ch_fork.gpd" using ($1/16384):($16) w lp ls 6 ti "{/Serif=15 верхняя оценка}", \
 "../../ch.gpd" using ($1/16384):($4) w l ls 7 ti "{/Serif=15 формула Хашина-Хилла}"
...
{({/Symbol n}_{zx}^{чис}-{/Symbol n}_{zx}^{ХХ})/{/Symbol n}_{zx}^{чис}}" font "Serif, 15"
set origin 0.12,0.54
set size 0.5, 0.45
set noxlabel 
#set xlabel "{S_B/S}" 
set noylabel
...
set grid x y
plot \
 "comparison_q_ch.gpd" u ($1):($4) w l ls 1 ti "" , \
 "comparison_s_ch.gpd" u ($1):($4) w l ls 2 ti "" , \
 "comparison_cr_ch.gpd" u ($1):($4) w l ls 3 ti "" , \
 "comparison_ci_ch.gpd" u ($1):($4) w l ls 4 ti ""# , \
