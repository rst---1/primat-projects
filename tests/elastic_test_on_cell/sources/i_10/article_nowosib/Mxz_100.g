...
set output name."Mxz.png"
set multiplot
set origin 0.01,0.01
set size 0.99, 0.99
set xrange [ 0 : 1 ]
set yrange [ 0.04 : 40.4 ]
set xtics 0, 0.1, 1#0,1024,16384
set ytics 0.04, 8.0, 40.4
set key spacing 2.5
set key top right
...
set ylabel "Модуль сдвига G_{xz}" font "Serif, 30"# offset 2
#{/Symbol m_{/Nosymbol xz}}
...
plot \
 dir."mata-quadrate.gpd" using ($1/16384):($12) w l ls 1 ti "{/Serif=15 квадрат}",\
 dir."mata-shell.gpd" using ($1/16384):($12) w l ls 2 ti "{/Serif=15 трубка}",\
 dir."mata-cross.gpd" using ($1/16384):($12) w l ls 3 ti "{/Serif=15 крестовина}", \
 dir."mata-circ.gpd" using ($1/16384):($12) w l ls 4 ti "{/Serif=15 круг}", \
 "../../ch_fork.gpd" using ($1/16384):($12) w lp ls 5 ti "{/Serif=15 нижня оценка}" , \
 "../../ch_fork.gpd" using ($1/16384):($24) w lp ls 6 ti "{/Serif=15 верхняя оценка}", \
 "../../ch.gpd" using ($1/16384):($12) w l ls 7 ti "{/Serif=15 формула Хашина-Хилла}"
...
{(G_{xz}^{чис}-G_{xz}^{ХХ})/G_{xz}^{чис}}" font "Serif, 15"
...
set grid x y
plot \
 "comparison_q_ch.gpd" u ($1):($12) w l ls 1 ti "" , \
 "comparison_s_ch.gpd" u ($1):($12) w l ls 2 ti "" , \
 "comparison_cr_ch.gpd" u ($1):($12) w l ls 3 ti "" , \
 "comparison_ci_ch.gpd" u ($1):($12) w l ls 4 ti ""# , \
