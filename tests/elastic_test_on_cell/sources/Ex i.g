...
set ylabel "Модуль Юнга {E_x}" font "Monospace, 25" offset 2

plot \
 "mata-quadrate i 10.gpd" using ($1/16384):($2/DEVIDER) w l ti "{/Monospace=20 3 квадрат}" lt 1 lw WEIGHT,\
 "mata-shell i 10.gpd" using ($1/16384):($2/DEVIDER) with lines ti "{/Monospace=20 4 трубка }" lt 2 lw WEIGHT,\
 "mata-cross i 10.gpd" using ($1/16384):($2/DEVIDER) with lines ti "{/Monospace=20 5 крестовина }" lt 3 lw WEIGHT, \
 "mata-circ i 10.gpd" using ($1/16384):($2/DEVIDER) with lines ti "{/Monospace=20 6 круг }" lt 6 lw WEIGHT, \
 "mata-quadrate i 10.gpd" using ($1/16384):(mean(1,16384-$1)/DEVIDER) w l ti "{/Monospace=20 1 среднее арифметическое, он же {E_z}}" lt 4 lw WEIGHT,\
 "mata-quadrate i 10.gpd" using ($1/16384):(serial(16384-$1)/DEVIDER) w l ti "{/Monospace=20 2 среднее гармоническое}" lt 5 lw WEIGHT
