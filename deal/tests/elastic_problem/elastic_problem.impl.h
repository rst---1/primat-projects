...
//    std::array<double, dim> res = this->content(p);
    std::array<double, dim> res = {p(0), 0.0};
...
//    this->boundary_values .set (boundary_values);
    for (typename ElasticProblemSup<dim>::BoundaryValues bv : boundary_values)
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>::solved ()
{
    printf("1\n");
    REPORT setup_system ();
    printf("2\n");
...
template<uint8_t dim>
void ElasticProblem<dim>::print_result (const std::string &file_name)
{
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>::setup_system ()
{
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>::assemble_system ()
{
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>::apply_boundary_values ()
{
...
//    printf("3.2\n");
    ...
    for (typename ElasticProblemSup<dim>::BoundaryValues bv : this->boundary_values)
    {
        if (bv.type _is ElasticProblemSup<dim>::BoundaryValues::Dirichlet)
        {
        puts("Dirichlet");
            std::map<unsigned int,double> list_boundary_values;
            ...
            dealii::VectorTools::interpolate_boundary_values (
                    ...
                    bv.function, //BoundaryValues<dim>(),// 
                    list_boundary_values);
        ...
        else if (bv.type _is ElasticProblemSup<dim>::BoundaryValues::Neumann)
        {
        puts("Neumann");
        // printf("Neuman %ld\n", bv.boundari_indicator);
            dealii::Vector<double> tmp (this->system_equations.b.size());
            std::set<u8> b_id;
            b_id.insert(bv.boundari_indicator);
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>::solve_system_equations ()
{
    ...
    solver .solve (
           ...
           //);
    //for (auto i : this->system_equations.b)
...
template<uint8_t dim>
prmt::Report ElasticProblem<dim>:: output_results ()
{
...
//        printf("x[%ld]=%f \n", i, this->system_equations.x(i));
    ...
    std::ofstream output (output_file_name + "move.gpd");
